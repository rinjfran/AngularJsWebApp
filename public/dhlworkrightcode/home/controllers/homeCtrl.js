    ...
    homeService.getAllActivities().then(function(response) {
      if (response.status === 200) {
        ...
        for(var i=0;i<$scope.allActivities.length;i++){
          if($scope.allActivities[i].rank==="good"){
            $scope.allActivities[i].progressBarClass = "progress-bar progress-good";
            $scope.progressBarColor = "progress-bar progress-good";
          }else if($scope.allActivities[i].rank==="bad"){
            $scope.allActivities[i].progressBarClass = "progress-bar progress-bad";
            $scope.progressBarColor = "progress-bar progress-bad";
          }else{
            $scope.allActivities[i].progressBarClass = "progress-bar progress-normal";
            $scope.progressBarColor = "progress-bar progress-normal";
          }
        }
      } else {
        $rootScope.hideLoading();
